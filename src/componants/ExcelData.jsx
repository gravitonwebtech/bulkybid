...
export default function ExcelData()  {
    ...
    const handleFileUpload = () => {
        ...
        axios.post('http://localhost:8000/api/upload_deals/', formData, {
            headers: {
    ...
    const fetchDeals = () => {
        axios.get('http://localhost:8000/api/get_deals/')
            .then(response => {
    ...
    const searchItems = (searchValue) => {
        ...
        if (searchValue !== '') {
            const filteredData = deals.filter((item) => {
                return Object.values(item.fields).join('').toLowerCase().includes(searchValue.toLowerCase());
            });
